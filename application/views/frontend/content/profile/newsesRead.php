  ...
  <div id="contentWrapper"><!--beginning contentWrapper-->
	...
	<article class="item-content-blog">
		<?php
			...
			else if($news->id_rcontent == 5) $link = "newses";
			else if($news->id_rcontent == 6) $link = "events";
		?>
		<div class="photos-blog"><!--beginning photos-blog--> 
			<?php echo anchor("$controller/$link/$news->id_content", '<span class="roll-img-blog"></span><img src="'. $dir_uploads.$news->picture_content .'" style="width:650px;height:300px;" alt="">'); ?>
		</div>
		<div class="intro-text-blog-single"><!--beginning intro-text-blog-->
			<?php echo anchor("$controller/$link/$news->id_content", '<h3 class="title-three-4"><span>'.$news->name_content.' </span></h3>', array("style"=>"text-decoration:none;")); ?>
					...
					<div class="content-comment-one<?=($comment->parent_comment != 0)? "-reply":""?>"><!--beginning content-comment-one-->
						<div class="line-one">
							...
							<div class="pos-reply"><img src="<?=$dir_images?>curved_arrow_24x18.png" alt="">
								<a href="<?php echo base_url() . "$controller/newses/$id_content/$comment->id_comment" . $url_suffix; ?>#comment" title="Reply">Reply</a>
							</div>
						</div>
		...
		<div class="new-page">
			<?php
				...
				$next = "";
				$i = 0;
				foreach($newses_nps as $newses_np):
					$crnt = $newses_np->id_content;
					$next = $newses_np->id_content;
