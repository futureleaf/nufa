  ...
  <div id="contentWrapper"><!--beginning contentWrapper-->
	...
	<article class="item-content-blog">
		<?php
			...
			$link = "";
			if($notification->id_rcontent == 1) $link = "newses";
			else if($notification->id_rcontent == 2) $link = "articles";
			else if($notification->id_rcontent == 3) $link = "achievements";
			else if($notification->id_rcontent == 4) $link = "notifications";
			else if($notification->id_rcontent == 5) $link = "notifications";
			else if($notification->id_rcontent == 6) $link = "events";
		?>
		<div class="photos-blog"><!--beginning photos-blog--> 
			<?php echo anchor("$controller/$link/$notification->id_content", '<span class="roll-img-blog"></span><img src="'. $dir_uploads.$notification->picture_content .'" style="width:650px;height:300px;" alt="">'); ?>
		</div>
		<div class="intro-text-blog-single"><!--beginning intro-text-blog-->
			<?php echo anchor("$controller/$link/$notification->id_content", '<h3 class="title-three-4"><span>'.$notification->name_content.' </span></h3>', array("style"=>"text-decoration:none;")); ?>
			
			<p class="doc-post"> 
				<span class="time-post"><img src="<?=$dir_images?>calendar_alt_fill_16x16.png" alt=""> <?php echo anchor("$controller/$link/$notification->id_content", $this->method->dateFromDatabaseText($notification->ud_content)); ?></span> 
				<span class="admin-post">
					<?php echo ($notification->gender != 0 || $notification->gender == null)? '<img src="'.$dir_images.'user_12x16_grey.png" alt="">' : '<img src="'.$dir_images.'user-woman_12x14_grey.png" alt="">';?> 
					<?php echo anchor("$controller/$link/$notification->id_content", ($notification->id_user ==null)?"Admin":$notification->full_name); ?>
				</span> 
				<span class="comment-post"><img src="<?=$dir_images?>comment_stroke_16x14.png" alt=""> <?php $count_comments = $this->mdl_comment->count_id_content_records($notification->id_content); echo anchor("$controller/$link/$notification->id_content", ($count_comments == 1)?$count_comments . " Comment":$count_comments . " Comments"); ?></span> 
				<span class="category-post"><img src="<?=$dir_images?>tag_fill_16x16.png" alt=""> 
				<?= anchor("$controller/$link", $link)?>
				</span> 
			</p>
			<p><?php echo $notification->desc_content;?></p>
					...
					<div class="content-comment-one<?=($comment->parent_comment != 0)? "-reply":""?>"><!--beginning content-comment-one-->
						<div class="line-one">
							...
							<div class="pos-reply"><img src="<?=$dir_images?>curved_arrow_24x18.png" alt="">
								<a href="<?php echo base_url() . "$controller/notifications/$id_content/$comment->id_comment" . $url_suffix; ?>#comment" title="Reply">Reply</a>
							</div>
						</div>
		...
		<div class="new-page">
			<?php
				...
				$next = "";
				$i = 0;
				foreach($notifications_nps as $notifications_np):
					$crnt = $notifications_np->id_content;
					$next = $notifications_np->id_content;
					...
					$prvs = $notifications_np->id_content;
				endforeach;
				$prvs = prvs;
				$crnt = crnt;
