  ...
  <div id="contentWrapper"><!--beginning contentWrapper-->
	...
	<article class="item-content-blog">
		<?php
			...
			else if($achievement->id_rcontent == 2) $link = "achievements";
			else if($achievement->id_rcontent == 3) $link = "achievements";
			else if($achievement->id_rcontent == 4) $link = "achievements";
			else if($achievement->id_rcontent == 5) $link = "achievements";
					...
					<div class="content-comment-one<?=($comment->parent_comment != 0)? "-reply":""?>"><!--beginning content-comment-one-->
						<div class="line-one">
							...
							<div class="pos-reply"><img src="<?=$dir_images?>curved_arrow_24x18.png" alt="">
								<a href="<?php echo base_url() . "$controller/achievements/$id_content/$comment->id_comment" . $url_suffix; ?>#comment" title="Reply">Reply</a>
							</div>
						</div>
						<div class="line-two">
							<div class="time"><img src="<?=$dir_images?>clock_16x16.png" alt="dfdf"><?php echo $this->method->dateFromDatabaseText($comment->cd_comment); ?></div>
						</div>
		...
		<div class="new-page">
			<?php
				...
				$next = "";
				$i = 0;
				foreach($achievements_nps as $achievements_np):
					$crnt = $achievements_np->id_content;
					$next = $achievements_np->id_content;
					...
					$prvs = $achievements_np->id_content;
				endforeach;
				$prvs = prvs;
				$crnt = crnt;
